...
void loop() {
  ...
  // loop para enviar conexão ativa a cada 5 segundos
  currentMillis = millis();  
  if (currentMillis - heartbeatMillis >= 30000)
  {
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  // aborta chamada caso não tenha sinal
  if (payload[0] != '-' && payload[0] != '+') {
    mqttClient.publish(topicState, "@002");
    Serial.print("Valor sem sinal");
    return;
  }
  ...
  if (qtde_voltas >= 800  && qtde_voltas <= 899) {
    ...
    mqttClient.publish(topicState, "+899");
  ...
  if (qtde_voltas >= 700  && qtde_voltas <= 799) {
    ...
    mqttClient.publish(topicState, "+799");
  ...
  if (first_run) {
    ...
    mqttClient.publish(topicState, "@001");
    return;
  }
  ...
  if (qtde_voltas >= 600  && qtde_voltas <= 699) {
    ...
    mqttClient.publish(topicState, "+699");
