...
// ------------------------------------
  ...
  while (!ConexaoMQTT.connected()) {
    ...
    ConexaoMQTT.connect("nodemcu",mqttuser,mqttpass);
    delay(3000);
  }
...
// ------------------------------------
  ...
  String novosensor = "{\"expire_after\": \"600\", \"icon\": \"mdi:gesture-tap-button\", \"name\": \"Smart Plug\", \"state_topic\": \"nodemcu/smatplug/state\"}";
  ConexaoMQTT.publish("homeassistant/binary_sensor/nodemcu/smatplug/config",novosensor.c_str(),false);
  ...
  //   ConexaoMQTT.publish("nodemcu/smartplug/state","OFF");
  // } else if (LeituraSwitchD1Pin == 1) {
  //   UltimoValor = LeituraSwitchD1Pin;
  //   ConexaoMQTT.publish("nodemcu/smartplug/state","ON");
  // }
    UltimoValor = 0;
    ConexaoMQTT.publish("nodemcu/smartplug/state","OFF");
...
// ------------------------------------
  if (TimeCounter < 20) {
    ...
    ConexaoMQTT.publish("nodemcu/smartplug/state","ativo");
    // if (guardaEstado == 0) {
    //   ConexaoMQTT.publish("nodemcu/smartplug/state","ativo");
    // } else {
    //   ConexaoMQTT.publish("nodemcu/smartplug/state","ON");
