...
void setup() {
  ...
  String payloadConfig = "{\"name\": \"CortinaSala\", \"command_topic\": \"esp32/cortinasala/control\", \"state_topic\": \"esp32/cortinasala/state\"}";
  mqttClient.publish("esp32/cortinasala/config", payloadConfig.c_str(), true);
...
void loop() {
  ...
  //   if (guardState == LOW) {
      //  mqttClient.publish(topicState, "active");
  //   } else {
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  mqttClient.publish(topicControl, "received:");
...
void voltas(int qtde, int direcao)  // 1 = direita; -1 = esquerda
{
  int i = 0;
  int sentido = -1;
  mqttClient.publish(topicControl, "volta_inicio");
  // liga motor
  ...
  mqttClient.publish(topicControl, "volta_fim");
