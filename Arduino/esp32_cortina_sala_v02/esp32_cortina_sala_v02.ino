...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (qtde_voltas >= 800  && qtde_voltas <= 899) {
    ...
    mqttClient.publish(topicControl, "x899");
  ...
  if (qtde_voltas >= 700  && qtde_voltas <= 799) {
    ...
    mqttClient.publish(topicControl, "x799");
  ...
  if (first_run) {
    ...
    mqttClient.publish(topicControl, "@001");
    return;
  }
  ...
  if (qtde_voltas >= 600  && qtde_voltas <= 699) {
    ...
    mqttClient.publish(topicControl, "x699");
  ...
  if (qtde_voltas == 200) {
    ...
    mqttClient.publish(topicControl, "x200");
