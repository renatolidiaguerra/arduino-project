...
/*------------------------------------ Configuration --------------------------------------------------------------------------------------------*/
/*-----------------------------------------------------------------------------------------------------------------------------------------------*/
const char*         g_ssid = "warbrito";
const char*         g_password = "gb240820";
const char*         g_mqtt_server = "192.168.15.10";                           // MQTT Server IP, same of Home Assistant
const char*         g_mqttUser = "renatobrito";                                 // MQTT Server User Name
const char*         g_mqttPsw = "gb240820";                                   // MQTT Server password
...
void loop() 
{
    ...
    if(g_InitSystem)
    {
        ...
        Serial.println("retornando do discovery");
    }
    ...
    if(millis() - g_Time > PERIOD_MILLSEC_500)  // Every 500 [msec]
    {
        ...
        if(g_count++ == 20 || g_SendMqttData) // Every 10 [sec] or if input status changed
        {
            ...
            ////////////////////////////////////////////////////////////////
            g_Temperature = 77; //g_dht.readTemperature();
            g_Humidity = 88;    //g_dht.readHumidity();
...
void MqttHomeAssistantDiscovery()
{
    ...
    if(g_mqttPubSub.connected())
    {
        ...
        Serial.println(" ");
        Serial.println("debug 05.1");

        serializeJson(payload, strPayload);
        Serial.println("debug 06");

        g_mqttPubSub.publish(discoveryTopic.c_str(), strPayload.c_str());
        Serial.println("debug 07");
